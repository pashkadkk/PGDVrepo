...
public class SuperZombie extends GameElement implements EnemiesWithDirectionalLoop {
	...
	public SuperZombie() throws SlickException {
		...
		// defines a sprite for superzombie object
	}
				...
				if(this.position.x+i==targetX && this.position.y+k==targetY )
				{
					if(i!=0 && this.position.x+i!=1)
					{
						...
						if (isInteracting(Wall.class))
							this.position.x-=i; 
						// if there is a target nearby and no wall between superzombie and a specified target X and Y - the superzombie changes it's position
						this.position.y+=k;
						if (isInteracting(Wall.class))
							this.position.y-=k;
						// if there is a target nearby and no wall between superzombie and a specified target X and Y - the superzombie changes it's position
	...
	public void update() {
		...
		// calls for a loop for changing the direction of superzombie starting with 'startingdirection'. Later on it changes based on superzombie location.
